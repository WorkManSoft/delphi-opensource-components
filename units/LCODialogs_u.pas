...
unit LCODialogs_u;
{
  ...
  2011-09-04 Delphi XE har även   , 'Close' i deklarationen.. men denna unit är kanske
  overkill numera????
...
//International section
  ...
  CaptionsR: array[TMsgDlgType] of string = ( 'Ïðåäóïðåæäåíèå', 'Îøèáêà',
    'Èíôîðìàöèÿ', 'Ïîäòâåðæäåíèå', '' );
  {$IFDEF A_D3}
  ButtonCaptionsR: array[TMsgDlgBtn] of string = (
    'Äà', 'Íåò', 'OK', 'Îòìåíà', 'Ñòîï', 'Ïîâòîð', 'Ïðîïóñòèòü', 'Âñå',
    'Íåò äëÿ âñåõ', 'Äà äëÿ âñåõ', 'Ñïðàâêà'{$IFDEF VER220}, 'Close'{$ENDIF});
  {$ELSE}
  ButtonCaptionsR: array[TMsgDlgBtn] of string = (
  ...
  ButtonCaptionsP: array[TMsgDlgBtn] of string = (
    'Sim', 'Não', 'OK', 'Cancela', 'Aborta', 'Tenta Novamente', 'Ignora', 'Tudo',
    'Não para Tudo', 'Sim para Tudo', 'Ajuda'{$IFDEF VER220}, 'Close'{$ENDIF});
  {$ELSE}
  ButtonCaptionsP: array[TMsgDlgBtn] of string = (
    'Sim', 'Não', 'OK', 'Cancela', 'Aborta', 'Tenta Novamente', 'Ignora', 'Tudo', 'Ajuda');
  {$ENDIF}


  //*** Turkish
  sCancelCaptionT = 'Vazgeç';
  CaptionsT: array[TMsgDlgType] of string = ('Uyarý', 'Hata',
    'Bilgi', 'Doðrulama', '');
  {$IFDEF A_D3}
  ButtonCaptionsT: array[TMsgDlgBtn] of string = (
    'Evet', 'Hayýr', 'TAMAM', 'Vazgeç', 'Durdur', 'Tekrar', 'Ýptal', 'Hepsi',
    'Hepsine hayýr', 'Hepsine evet', 'Yardým'{$IFDEF VER220}, 'Close'{$ENDIF});
  {$ELSE}
  ButtonCaptionsT: array[TMsgDlgBtn] of string= (
    'Evet', 'Hayýr', 'TAMAM', 'Vazgeç', 'Durdur', 'Tekrar', 'Ýptal', 'Hepsi', 'Yardým');
  {$ENDIF}
  ...
  ButtonCaptionsF: array[TMsgDlgBtn] of string = (
    'Oui', 'Non', 'OK', 'Annuler', 'Abandonner', 'Réessayer', 'Ignorer', 'Tout',
  ...
  CaptionsGr: array[TMsgDlgType] of string = ('Ðñïóï÷Þ!', 'ËÜèïò',
    'Ðëçñïöïñßá', 'Åðéâåâáßùóç', '');
  {$IFDEF A_D3}
  ButtonCaptionsGr: array[TMsgDlgBtn] of string = (
    'Íáß', '¼÷é', 'ÅíôÜîåé', '¢êõñï', '¢êýñùóç', 'ÐñïóðÜèåéá', 'Áãíþçóç', '¼ëá',
    '¼÷é óå üëá', 'Íáß óå üëá', 'ÂïÞèåéá'{$IFDEF VER220}, 'Close'{$ENDIF});
  {$ELSE}
  ButtonCaptionsGr: array[TMsgDlgBtn] of string= (
    'Íáß', '¼÷é', 'ÅíôÜîåé', '¢êõñï', '¢êýñùóç', 'ÐñïóðÜèåéá', 'Áãíþçóç', '¼ëá', 'ÂïÞèåéá');
  {$ENDIF}
...
//*** Swedish
  ...
  CaptionsSE: array[TMsgDlgType] of string = ('Varning'{'Warning'}, 'Fel'{'Error'},
    'Information', 'Bekräfta'{'Confirm'}, '');
  {$IFDEF A_D3}
  ButtonCaptionsSE: array[TMsgDlgBtn] of string = (
    'Ja'{'Yes'}, 'Nej'{'No'}, 'OK', 'Avbryt'{'Cancel'}, 'Avbryt'{'Abort'},
    'Försök Igen' {'Retry'}, 'Ignorera'{'Ignore'}, 'Alla'{'All'},
    'Nej till alla'{'No to all'}, 'Ja till alla'{'Yes to all'}, 'Hjälp'{'Help'}{$IFDEF VER220}, 'Close'{$ENDIF});
  {$ELSE}
  ButtonCaptionsSE: array[TMsgDlgBtn] of string= (
    'Ja', 'Nej', 'OK', 'Avbryt', 'Avbryt', 'Försök Igen'{?}, 'Ignorera', 'Alla', 'Hjälp');
//    'Yes', 'No', 'OK', 'Cancel', 'Abort', 'Retry', 'Ignore', 'All', 'Help');
  {$ENDIF}
